...
public class UserCredentialValidator {
    ...
    public void validatePassword(String password, Errors errors) {
        ...
        // Assuming a valid password needs to contain at least one uppercase letter, one
        // lowercase letter, one number and one special character
        if (!(containsLowerCase(password) && containsUpperCase(password) && containsSpecialCharacter(password)
                && containsNumber(password))) {
            System.out
                    .println("Password must have at least one upper case, one lower case, and one special character.");
    ...
    public void validateUserName(String username, Errors errors){
        ...
        if (username.isEmpty()) {
            ...
            errors.reject("Invalid.username.null");
        }

        // Check if username exits
        if (userRepository.existsByUsername(username)) {
            System.out.println("Username is already registered.");
            errors.reject("Invalid.username");
        }
    }
